...
******* Units
******** Def: If R is a unital ring then an element a \in R is called a unit if there exists an element b \in R such that ab=1=ba. :definition:
...
Assume a,b \in Z such that (a + pZ)(b + pZ) = ab + pZ = pZ then ab \in pZ
so p divides ab.  As p is a prime either p | a or p |b so either a \in pZ or b \in
pZ so either a + pZ = pZ or b + pZ = pZ.
******** If n \gt 0 is non-prime then Z/nZ has zero divisors. 
If n = 8 then (2 + 8Z)(4 + 8Z) = 8 + 8Z = 8Z
...
If no such n exists then we say R has characteristic 0 and otherwise it has
characteristic n.  We denote by char(R) the characteristic.
...
****** Lecture 28
[2019-04-12 Fri]
******* Primes in the Gaussian Integer
******** Recall U(Z[i]) = {1, -1, i, -i}
******** We want to find all primes hence irreducibles in the UFD Z[i]
******** Question: Is p > 0 a prime integer irreducible in Z[i]?
******** Let p = f_1... f_n be a decomposition into irreducibles then p^2 = d(f_1)...d(f_n)
******** As Z is a UFD we have n = 2 because d(f_i) \ne 1 as f_j is not a unit.
******** Either n = 1 and d(f_1) = p^2 a n=2 and d(f_1) = d(f_2) = p
******** Lemma: If p is a prime integer then exactly one of the following holds
********* Either p \in Z[i] is irreducible
********* p = a^2 + b^2 for some a,b \in Z 
********** Proof
********** If p is reducible then p = fg with f,g \in Z[i] irreducibles and d(f) = d(g) = p
********** So if f = a + bi then p = d(f) = a^2 + b^2
********** Conversely if p = a^2 + b^2 = (a + bi)(a - bi)
********** d(a + bi) = d(a - bi) = p so a +/- bi are irreducible
******** Corollary: 
If p > 0 is an odd prime integer and p = a^2 + b^2, for some a,b \in Z, then p =
\pi\pi_bar where \pi = a + bi and \pi_bar = a - bi are non-associate irreducible elements
of Z[i] with d(\pi) = d(\pi_bar) = p
********* There are only four ways they can be associates
********** a + bi = a - bi \to b = 0 and p = a^2
********** a + bi = -a +bi \to a = 0 and p = b^2
********** a + bi = -b -ai \to a = -b and p = 2a^2
********* All of these contradict our assumption.
******** Corollary
If p > 0 is a prime integer with p \equiv 3 (mod 4) the p is irreducible in Z[i]
********* Assume p = a^2 + b^2, for a, b \in Z, then reducing mod 4 we get 3 \equiv a^2 + b^2 (mod 4).
********* Squares mod 4 are 0^2 \equiv 2^2 \equiv 0 and 1^2 \equiv 3^2 \equiv 1 so there is no solution mod 4
******** Theorem: If p > 0 is a prime integer then the following are equivalent
********* p = 2 or p \equiv 1 (mod 4)
********* The 4_th cyclotomic polynomial \phi_4 = x^2 + 1 \in F_p[X] is irreducible
********* p = a^2 + b^2 with a,b \in Z
********* Proof:
****** Lecture 29
[2019-04-15 Mon]
******* Proof:
******** Assume \alpha \in Z[i] is prime theb d(\alpha) = \alpha\alpha_bar so \alpha | d(\alpha) in Z[i]
******** As Z is a UFD we can write d(\alpha) = p_1... p_n as a product of irreducible hence prime elements p_i \in Z
******** As \alpha is prime we have \alpha | p_i for some p_i.
******** Hence it suffices to consider the decomposition of each prime integer in Z[i].
******** 2= -i(1 + i)^2
******* Remark: The prime: 
******** 2 is called ramified because 2 = -i(1 + i)^2 repeated
******** p \equiv 1 (mod 4) is split in Z[i]
******** p \equiv 3 (mod 4) is inert in Z[i]
******* Quotients
******** If \pi \in Z[i] is prime then k_\pi = Z[i]/\pi Z[i] is a field, what kind of field
******** Lemma: We have an isomorphism of rings Z[X]/<x^2+1> = Z[i]
******** Proof:
********* Consider the evaluation map \epsilon_i : Z[x] \to C given by \epsilon_i(f) = f(i). This is a ring isomorphism
********* Assume f \in Ker(\epsilon_i). 
********* Then by the division algorithm applied in Q[x] there exists polynomial q,r \in Q[X] such that f = q(x^2 + 1) + r where either r = 0 or deg(r) < 2
********* Assume r \ne 0 then r = aX + b with a,b \in Q
********* Now r(i) = 0 so ai + b = 0 but this implies a = b - 0
********* Check now that q \in Z[X].
******** Let p > 0 be a prime then under this isomorphism the ideal pZ[i] corresponds to the ideal <x^2 + 1> \sub <p, x^2 + 1> \sub Z[X].
******** Want to understand Z[i]/pZ[i]
********* This is the same as understanding (Z[x]/<x^2 + 1>)/(<p, x^2 + 1>/<x^2 + 1>) = Z[x]/<p, x^2 + 1> = (Z[x]/<p>)/(<p, x^2 + 1>/<p>) = F_p[X]/<x^2 + 1>
******** Proposition: Assume \pi \in Z[i] is prime the k_\pi is a finite field of size d(\pi).
******** Proof:
********* Note \pi | p, some prime integer p > 0 so pZ[i] \sub \pi Z[i] \sub Z[i]
********* If p = 2 then x^2 + 1 = (x + 1)^2
********* and <x + 1> is maximal in F_p[X]
********* and F_p[X]/<x+1> = F_p. In this case d(\pi)=2=p
********* If \pi = p with p \equiv 3 (mod 4) then \phi_4 = x^2 + 1 \in F_p[X] is irreducible so k_\pi = Z[i]/pZ[i] = F_{p^2} and d(\pi)=d(p)=p^2 
********* Finally consider \pi\pi_bar = p > 0 and p \equiv 1 (mod 4)
********** In this case \phi_4 is reducible over F_p so there exists a zero \epsilon \in F_p.
********** The ideal <x - \epsilon> \sub F_p[X] contains <x^2+1> and F_p[X]/<x-\epsilon> = Z[i]/\piZ[i]
********** Observe F_p[x]/<x-\epsilon> and d(\pi)=p
******** The field k_\pi is often called the residue field.
****** Lecture 30
[2019-04-19 Fri]
******* Extension Fields
******** A field E is an extension field of a field F if F \sub E and the operations on F are obtained by restricting those on E. :definition:
********* Here F is a subfield of E
******** Examples
********* If E is a field then E is an extension of its ground field F, which is the unique smallest subfield containing {n1_E | n \in Z}
******** How big can an extension be?
******** The degree of a field extension F \sub E is defined to be [E:F] = dim_F(E) :definition:
******** Remark: If F \sub E is a field extension then E is an F-vector space
******** Example:
********* [C:R] = 2 because C = {a + bi | a,b \in R}
********** So {1, i} is a basis of C an R-vector space
********* [R:Q] = \infin (the continuum)
********* Let f = x^2 - x + 1 \in F_3[X] then this is irreducible so F = F_3[X]/<f> is a field.
********** Let \alpha = X + <f> then we know that F = {a\alpha^2 + b\alpha + c\alpha^0 | a,b,c \in F_3}
********** We have {\alpha^0, \alpha, \alpha^2} \sub F is a basis as an F_3-vector space so [F:F_3] = 3 = deg(f)
********** A field F_27^3 then [F_27^3:F_27] = 3 and [F_27^3:F_3] = 9
********* Lemma: Let F be a field of characteristic p > 0. Then F is finite if and only if n = [F:F_p] < \infin.
********** If n < \infin then the additive group F^{+} is isomorphic to Z^{n}p = Z_p directsum n times and |F| = p^n
********** Proof:
*********** If n = [F:F_p] then F has a finite basis B = {e_1,..., e_n} as an F_p-vector space.
*********** This means we have an isomorphism of vector spaces F_p^n \to F defined by (a_1,..., a_n) \to a_{1}e_1 + ... + a_{n}e_n
*********** As this map is bijective we have |F| = |F_p^n| = p^n < \infin and it is an isomorphism of additive groups
*********** We have F^{+} = Z_p^n.
*********** Conversely if F is finite then any basis as an F_p-vector space must be finite.
********* Theorem (Kronecker): Assume F is a field and f \in F[X] is a non-zero, non-unit then there exists an extension field E of F and a zero \alpha \in E of f 
********** Proof:
*********** As F[X] is a UFD and f is a non-zero non-unit there exists an irreducible element p \in F[X] such that p | f. 
*********** As p is irreducible E = F[X]/<p> is a field and \alpha = X + <p> is a zero of p
*********** As p divides f \alpha is also a zero of f.
********* Example:
********** Consider f = x^3 - 1_Q \in Q[X] then f = (x - 1_Q)\phi_3 where \phi_3 = x^2 + x + 1_Q
********** Let's get a zero of \phi_3
********** Set F = Q[X]/\phi_3 and let \omega = X + <\phi_3>
********** So \omega^2 + \omega + 1_F = (X^2 + <\phi_3>) + (x + <\phi_3>) + (1_Q + <\phi_3>) = x^2 + x + 1_Q + <\phi_3> = <\phi_3>
********** Also we have \omega^3 - 1_F = 0_f so \omega is a non-trivial third root of unity.
********* An element \zeta in a field in a field F is an n^th root of unity if \zeta^N = 1_F. :definition:
********** We say it is primitive if \zeta^d \ne 1_f for any d | n.
*********** Primitive means \zeta is a generator of the group {\zeta \in U(F) | \zeta^n = 1}
******* Splitting Fields
******** The polynomial \phi_3 = x^2 + x + 1 \in Q[X] has no zeros in Q hence no linear factors.
******** However in our extension field F = Q[X]/<\phi_3> we have
********* (X - \omega)(X - \omega^2) = X^2 - (\omega^2 + \omega)X + 1 = x^2 + x + 1
******** So in F[x] the polynomial \phi_3 splits as a product of linear factors
********* X^3 - 1 = (X - 1)(X - \omega)(X - \omega^2)
******** Assume F \sub E is a field extension. Given a_1, ..., a_n \sub E the smallest subfield of E containing both F and {a_1, ..., a_n}. :definition:
********* We say f \in F[X] splits in E is there are elements a \in F and a_1, ..., a_n \in E such that f = a(X - a_1)...(X - a_n)
********* We say E is a splitting field for f over F if E = F(a_1, ..., a_n)
...
**** HOMEWORK Zybooks reading.                                   :comporg: 
     DEADLINE: <2019-04-23 Tue 23:59 +7d>
     :PROPERTIES:
     :LAST_REPEAT: [2019-04-18 Thu 12:38]
     :END:
     - State "DONE"       from "HOMEWORK"   [2019-04-18 Thu 12:38]
...
**** HOMEWORK Simulation Project 5                                  :comporg: 
     DEADLINE: <2019-04-25 Thu 17:00>
**** HOMEWORK Assembly Project 5                                    :comporg: 
     DEADLINE: <2019-04-30 Thu 17:00>
...
**** HOMEWORK Abstract Algebra Homework                          :algebra: 
     DEADLINE: <2019-04-26 Fri 11:00 +7d>
     :PROPERTIES:
     :LAST_REPEAT: [2019-04-19 Fri 09:15]
     :END:

     - State "DONE"       from "HOMEWORK"   [2019-04-19 Fri 09:15]
     - State "DONE"       from "HOMEWORK"   [2019-04-18 Thu 12:38]
